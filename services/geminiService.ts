...
const getAIClient = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    const runtimeKey = win.ENV?.API_KEY || win.ENV?.REACT_APP_API_KEY || win.REACT_APP_API_KEY;
  ...
  if (!key) {
    if (!(window as any)._hasWarnedKey) {
        ...
        console.log("Please ensure 'API_KEY' or 'REACT_APP_API_KEY' is set in your Cloud Run Variables & Secrets.");
