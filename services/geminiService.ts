...
const getAIClient = () => {
    // 1. Get the key from the global object created by the startup script.
    // This is the source of truth for the deployed application.
    const win = window as any;
    const runtimeKey = win.ENV?.GEMINI_API_KEY;

    let key = runtimeKey;
    
    // 2. Fallback to process.env (primarily for local dev/testing)
    if (!key) {
    ...
    if (!key || key === "YOUR_RUNTIME_GEMINI_KEY_PLACEHOLDER") {
        if (!(window as any)._hasWarnedKey) {
            ...
            console.log("Ensure 'GEMINI_API_KEY' is set in Cloud Run Variables & Secrets.");
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                temperature: 0.1,
                tools: [{ googleSearch: {} }],
                // responseMimeType: "application/json" is omitted because it often conflicts with Search Tool.
            }
        });
