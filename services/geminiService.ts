 ...
 * @param page - The pagination index (1-based). Used to vary the search results.
 * @param filters - Advanced search filters (date, sort, source).
 */
export const fetchLiveNews = async (category: string, page: number = 1, filters?: SearchFilters): Promise<Article[]> => {
  ...
  const prompt = `
    ...
    3. Do NOT wrap the JSON in markdown code blocks (like \`\`\`json). Return ONLY the raw JSON string.
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: prompt,
      config: {
        temperature: 0.3, // Slightly higher temperature for variety on pagination
        tools: [{ googleSearch: {} }],
      }
    });
    ...
    return parsedData.map((item: any, index: number) => {
       ...
       // Generate a few extra random images from the category pool for the carousel
       // We use a pseudo-random slice to get variety
       const extraImages = getUniqueCategoryImages(category, 3);
       
       return {
          id: `live-${Date.now()}-${page}-${index}`, // Unique ID including page
          title: item.title || "Untitled News",
          source: item.source || "Unknown Source",
          publishedAt: item.publishedAt || new Date().toISOString(),
          url: "https://google.com/search?q=" + encodeURIComponent(item.title), // Fallback URL if extraction fails
          ...
          imageUrl: mainImage,
          images: [mainImage, ...extraImages].slice(0, 3) // Ensure we have a nice set of 3 images
       };
    });
 ...
 * Finds articles relevant to the query to use as context.
 */
const retrieveContext = (query: string, activeArticle?: Article | null): Article[] => {
  if (activeArticle) {
    return [activeArticle];
  }
  return []; // In live mode, we rely more on Google Search if no article is active
};
