...
const getAIClient = () => {
  ...
  // 1. Process Env (Standard Node/Build)
  ...
  if (!key && typeof window !== 'undefined') {
    ...
    key = win.ENV?.API_KEY || win.ENV?.REACT_APP_API_KEY || win.REACT_APP_API_KEY;
  }
  ...
  if (!key) {
    if (!(window as any)._hasWarnedKey) {
        console.error("CRITICAL: No API Key found in process.env, window.ENV, or import.meta.env");
        console.log("Please add 'REACT_APP_API_KEY' or 'API_KEY' to your Cloud Run Variables & Secrets.");
